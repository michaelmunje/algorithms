...
def bst_to_list_level_by_level(head):
    q = Queue()
    q.enqueue(head)
    values = []
...
class TestSolution(unittest.TestCase):
    def test_stacks(self):
        a = [1, 2, 3, 4, 5, 6, 7, 8, 9]
        head = list_to_bst(a)
        self.assertEqual(bst_to_list_level_by_level(head), [5, 3, 8, 2, 4, 7, 9, 1, 6])
        a = [1, 2, 3, 4]
        head = list_to_bst(a)
        self.assertEqual(bst_to_list_level_by_level(head), [3, 2, 4, 1])
